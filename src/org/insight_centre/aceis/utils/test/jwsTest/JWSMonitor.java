...
public class JWSMonitor implements Runnable {

	@Override
	public void run() {
		int cnt = 0;
		while (!JWSTest.anyStreamStopped()) {
			try {
				Thread.sleep(5000);
			...
			cnt += 1;
			Double delay = JWSTest.getAvgDelay();
			// if (delay.equals(Double.NaN))
			JWSTest.latencyOverTime.put(cnt * 5, delay);
			// else
	...
	private void printResults() {
		...
		TreeMap<Integer, Double> sortedLatency = new TreeMap(JWSTest.latencyOverTime);
		long inputCnt = 0;
		for (Long i : JWSTest.inBytesMap.values()) {
			inputCnt += i;
		}
		System.out.println("Total inputs: " + inputCnt);
		System.out.println("Total outputs: " + JWSTest.outByteCnt);
		System.out.println("Delay over time series: " + sortedLatency);
		writeCSV(inputCnt, JWSTest.outByteCnt, sortedLatency);
		System.exit(0);
	}

	private void writeCSV(Long input, Long output, TreeMap<Integer, Double> sortedLatency) {
		String outputFile = "resultLog/jws/" + JWSTest.mode + ".csv";
