...
public class JWSCsparqlResultFormatter extends ResultFormatter {
	...
	public void update(GenericObservable<RDFTable> observed, RDFTable q) {
		...
		for (final RDFTuple t : q) {
			...
			if (!this.latestResults.contains(result)) {
				...
				JWSTest.outByteCnt += result.getBytes().length;
				...
				for (int i = 0; i < results.length; i += 2) {
					...
					SensorObservation so = JWSTest.obMap.get(obId);
					JWSTest.latencyMap.put(so, System.currentTimeMillis());
				}
				/*
