 ...
 */
public class EventRepository implements Cloneable {
	...
	public EventRepository clone() throws CloneNotSupportedException {
		EventRepository erClone = new EventRepository();
		for (Entry<String, EventDeclaration> en : this.eds.entrySet()) {
			erClone.getEds().put(en.getKey(), en.getValue().clone());
		}
		for (Entry<String, EventPattern> en : this.eps.entrySet()) {
			erClone.getEps().put(en.getKey(), en.getValue().clone());
		}
		erClone.setReusabilityHierarchy(this.reusabilityHierarchy.clone());
		return erClone;
