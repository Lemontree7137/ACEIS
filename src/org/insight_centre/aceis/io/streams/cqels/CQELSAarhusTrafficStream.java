...
public class CQELSAarhusTrafficStream extends CQELSSensorStream implements Runnable {
	...
	// foi.addProperty(m.createProperty(VirtuosoDataManager.ctPrefix + "hasFirstNode"), this.annotateNode(m, 1, data));
	// foi.addProperty(m.createProperty(VirtuosoDataManager.ctPrefix + "hasFirstNode"), this.annotateNode(m, 2, data));
	// observation.addProperty(m.createProperty(VirtuosoDataManager.ssnPrefix + "featureOfInterest"), foi);
	//
	// }
	...
	private void annotateObservation(Model m, String pStr, AarhusTrafficObservation data) {
		Resource observation = m.createResource(VirtuosoDataManager.defaultPrefix + data.getObId());
	...
	private void annotateProperty(Model m, Resource observation, String observedProperty) {
		...
		observation.addProperty(m.createProperty(VirtuosoDataManager.ssnPrefix + "observedProperty"), property);
	...
	private void annotateSensor(Model m, Resource observation, AarhusTrafficObservation data) {
		...
		observation.addProperty(m.createProperty(VirtuosoDataManager.ssnPrefix + "observedBy"),
				m.createResource(this.ed.getSensorId()));
	...
	private void annotateValue(Model m, Resource observation, String observedProperty, AarhusTrafficObservation data) {
		...
		if (observedProperty.contains("Speed"))
	...
	protected SensorObservation createObservation(Object objData) {
		try {
			...
			ACEISScheduler.getObMap().put(data.getObId(), data.getSysTimestamp().getTime());
			// logger.info("putting: " + data.getObId());
			return data;
	...
	protected List<Statement> getStatements(SensorObservation data) throws NumberFormatException, IOException {
		...
		if (ed != null)
			...
			// test purposes only
			this.annotateObservation(m, m.createResource(VirtuosoDataManager.ctPrefix + "EstimatedTime").toString(),
					(AarhusTrafficObservation) data);
		return m.listStatements().toList();
	}

	public void run() {
		logger.debug("Starting sensor stream: " + this.getURI() + " " + this.startDate + ", " + this.endDate);
		try {
			...
			while (streamData.readRecord() && !stop) {
				...
				try {
					...
					logger.error("Exception while sending traffic observations");
				}
				this.messageCnt += 1;
